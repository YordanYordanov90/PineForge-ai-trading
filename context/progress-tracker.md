...
- **Complete**: Strategy Templates Library (`59`), Strategy Assumptions Block (`60`), Research → Generate Pipeline (`61` + 61.1 + 61.2 script attach)
- **Still planned**: Strategy DNA Fingerprint (`62`), Strategy Comparison Reports (`63`), Strategy Variants Quick-Generate (`64`), Quality Progression Tracker (`65`), Strategy Snapshot Export (`66`), Contextual Tips in Forge (`67`), Keyboard Power User Mode (`68`)
...
Phase 7: `59` (Strategy Templates Library), `60` (Strategy Assumptions Block), and `61` (Research → Generate Pipeline + 61.1 no-script banner) shipped. Next: `62` (Strategy DNA Fingerprint) and later Phase 7 items. Optional Phase 4 hardening
...
| `60` | Strategy Assumptions Block | **Complete** (parseAssumptionsBlock in lib/ai/parse-assumptions.ts; prompt extended in pine-generate-system.ts; client-side extraction + clean script in useScriptGeneration; SavedScript + metadata round-trip; AssumptionsSection + BreakdownTab in Breakdown output tab with amber styling + empty state; health-score prompt cross-references assumptions; npm run build clean) |
...
- `60-strategy-assumptions-block.md` — Phase 7 Priority 2 complete. New pure `parseAssumptionsBlock()` (lib/ai/parse-assumptions.ts); prompt extended (pine-generate-system.ts); client-side extraction + cleanScript in useScriptGeneration (never in copy/download); AssumptionsSection (AlertCircle, amber bullets matching Common Failure Modes, exact empty state) inside existing Breakdown tab (via BreakdownTab wrapper); health-score prompt cross-references assumptions; SavedScript + ScriptMetadata jsonb + mappers + builders + lineage sync; `npm run build` clean. Backward-compatible for pre-60 scripts. Post-review fix: output shell `terminal-code-surface--panel` restores vertical scroll (base surface used `overflow:hidden`); Breakdown/Checklist use parent scroll + abort explain fetch on tab blur to avoid stream-lock 409. ✅
- `61.1-research-no-script-banner.md` — Addition to 61 (Research → Generate). Pure UI change in `components/forge/ForgeChat.tsx`: added `activeScriptId` state (hydrated from `SavedConversation.scriptId` on load + reset on error/empty); `showNoScriptBanner` derived for `isResearchConv && scriptId == null`; non-dismissable amber informational banner (matches `/generate` research pre-fill + ui-context amber tokens) rendered in two positions per spec — full card above `ForgeEmptyState` suggestion chips for empty research threads, slim pill above composer input for active threads with messages. Directs users to `/generate` to start research with script context. No backend, no new routes, no dismiss logic (state-driven only). `npm run build` + lint clean. ✅
- `61.2-research-script-attach.md` — Full attach/change/detach for research threads. Backend: extended `updateConversationSchema` (optional `scriptId: number | null`), new `updateConversationScriptId` + exported `verifyScriptOwnership` in `lib/db/agent-conversations.ts`, PATCH route now handles partial `{title?, scriptId?}` with 403 on foreign script (reuses ownership check). Hook: `useForgeConversations.updateScriptId(id, scriptId)` with optimistic update + rollback + toast. UI: new `ResearchScriptBanner.tsx` (replaces 61.1 inline banner) + `ScriptPickerDialog.tsx` (shadcn Dialog, loads from `GET /api/scripts`, name/date/starred, client filter, max 20). Banner text exactly per spec ("No script attached. [Load from history]" / "Script: {name} · change · detach"). "Load from history" opens in-Forge picker (no /generate redirect). All 8 success criteria pass; `npm run build` + lint clean. Note: work performed on feature/phase-5 per user instruction (B); dedicated branch required before commit. ✅

## In Progress

## Next Up

- Phase 6 Forge Agent (`51`–`58`) complete. Phase 7 `59`–`61` (Templates, Assumptions Block, Research→Generate Pipeline + 61.1 + 61.2 script attach) shipped; next `62` (DNA Fingerprint).
